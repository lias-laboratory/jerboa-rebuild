...
\begin{enumerate}
	...
	\item enfin, chacun des n\oe uds d'orbite d'origine est relié au n\oe ud
				...
				connectée à un \creationnodeevent{} qui est lui-même connecté à
				\orbitnode{\(\orbit{0,1}\)} (respectivement \orbitnode{\(\orbit{0}\)}).
\end{enumerate}
%
Ce niveau du DAG
...
Un type d'origine est systématiquement calculé à partir d'un n\oe ud d'accroche
à cause de l'isomorphisme et de réécritures d'arcs. %
Toutefois, si l'orbite filtrée par le n\oe ud d'accroche peut, par sa
suppression, avoir une importance significative, celle-ci n'est pas forcément
une orbite d'origine. En revanche, elle appartient l'orbite filtrée par le n\oe
ud d'accroche est toujours connexe à l'orbite d'origine. %
Pour cette raison, nous intégrons une extension à la relation d'origine sous la
forme d'un chemin associé à un type d'orbite. %
Dans l'exemple que nous venons de voir, le chemin est nul car le n\oe ud
d'accroche filtrait systématiquement l'orbite d'origine. %
En effet, les règles de triangulation et d'extrusion n'ont qu'un seul n\oe ud
dans leurs membres gauches respectifs. %
%
\begin{figure}[htb!]
	\centering
...
\begin{figure}[htb!]
	\centering
	\includegraphics[height=40pt]{rule-face-collapse-n2-path-n0}
	\caption{Chemin entre \(n_{2}\) et \(n_{0}\) dans la contraction}
	\label{fig:reevaluation-base-regle-nomination-persistante-orbites-regle-contraction-chemin}
\end{figure}

Pour calculer un tel chemin, nous effectuons un parcours depuis le n\oe ud
enregistré par l'application jusqu'au n\oe ud d'accroche (voir le chemin en {\color{orange}orange} dans la figure~\ref{fig:reevaluation-base-regle-nomination-persistante-orbites-regle-contraction-chemin}). %
...
\begin{figure}[htb!]
	\centering
	\includegraphics[height=90pt]{rule-volume-face-extrusion-compact-n4-path-n6}
	\caption{Chemin entre \(n_{4}\) et \(n_{6}\) dans l'extrusion}
	\label{fig:reevaluation-base-regle-nomination-persistante-orbites-regle-extrusion-chemin}
\end{figure}
...
(figure~\ref{fig:reevaluation-base-regle-nomination-persistante-orbites-regle-extrusion-chemin}). %
