...
\begin{frontmatter}
	...
	%% Abstract
	\begin{abstract}
	  Rebuilding a 3D model is a tedious process, which involves number of trial
	  and error cycles. %
	  In order to alleviate this difficulty, current parametric modeling systems
	  propose a rebuild feature. %
	  This feature allows users to edit the construction process of a model and
	  regenerate it. %
	  ...
	  In this paper, we present a rule-based modeling system with a rebuild
	  mechanism. %
	  This mechanism allows users to edit construction processes through the
	  addition and deletion of operations while it tracks identified topological
	  parameters only. %

	  Our approach, based on the generalized maps and graph transformation rules
	  formalisms, make it possible to syntactically analyze operations and
	  tracks topological changes (creation, deletion, merging,\dots) during the
	  build and rebuild processes. %
