...
\begin{figure}[htb!]
	\centering
	\includegraphics[width=\linewidth]{sequence-extrusion-colonne-faces}
...
1-creationCarre()
2-S_extrusionColonne(PN%\textsubscript{5}%)
  Sequence:
...
script. %
Ainsi, les trois applications du script \texttt{S\_extrusionColonne} sont
...
Ainsi, l'historique d'une orbite créée dans une application de script sera
représenté selon le schéma suivant~: %
\[[\mathit{S\{R~noeud\}}]\] où \(S\) est le numéro d'application d'un script et
\(R\) le numéro d'application d'une règle dans un script. %
...
L'application \texttt{2\allowbreak{}\allowbreak{}-S\allowbreak{}%
	\_\allowbreak{}extrusion\-Colonne(PN\textsubscript{5})} entraîne les
...
l'application \texttt{2\allowbreak{}-\allowbreak{}S\_extrusion\-Colonne}. %
...
2-extrusionColonne(PN%\textsubscript{5}%)
  Sequence:
    1-extrusionAreteFace(PN%\textsubscript{5}%)
    2-extrusionAreteFace(PN%\textsubscript{15}%)
    3-extrusionAreteFace(PN%\textsubscript{23}%)
