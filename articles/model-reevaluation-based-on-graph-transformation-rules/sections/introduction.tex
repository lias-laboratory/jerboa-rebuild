...
\david{to our knowledge only one preliminary approach has attempted to use graph transformation rules to tackle this issue \cite{cardot2019persistent}.
...
This approach proposes to use \emph{History Records}(HR) to represent the history of [[XS : any]] topological entity designated in parametric specification and \emph{Matching Trees}(MT) to match this entity during reevaluation. This is an interesting initial theoretical approach based on graph transformation, but the history represented in HR is limited. In particular, no distinction is made between the entities at the origin of the designated entity and the evolution of the designated entity itself. Furthermore, [[XS : \sout{certain} some]] elements are omitted in HR (such as the history of entities at the origin of the designated entity), which can lead to [[XS : \sout{matching missing} miss matchings]] during reevaluation.
In [[XS : \sout{our contribution} this paper]], we propose to extend this work to integrate the complete history of topological entities.}
Our contribution is twofold. First, we widen the naming
problem studies to the rule-based graph transformation modeling systems. Second, we integrate
the mechanisms of reevaluation for parametric systems into Jerboa.
...
homogeneously characterized for all dimensions.  Most existing methods require  tracking 
  ...
  %from the parameters' modifications 
...
ones they originate from. Moreover, not only the naming problem is tackled within the usual framework of  parameter edition, but we also take the spe-cification edition (\textit{i.e.} adding, deleting and changing the order of operations) into account.
