...
public class MatchingTree {
  ...
  // LevelOrbitMT root;
  JerboaDart topoParameter; // the last dartID computed at the end of the
                            // matching process

  List<List<LevelEventMT>> leaves;

  /**
   * Data structure to match a history record against a GMap and build a model
   * from a history record.
   */
  public MatchingTree() { leaves = new ArrayList<>(); }

  public MatchingTree(JerboaRuleResult appResult, Application specEntry,
                      HistoryRecord HR) {
  ...
  public JerboaDart getTopoParameter() { return topoParameter; }

  public void addInitLevel(LevelEventHR level, Application prevApplication,
    ...
    nodeNameToDartID(appNumber, nodeName, appResult, prevApplication);
    ...
    if (!leaves.isEmpty()) {
      getLastLevel().get(0).getNextLevelOrbit().setNextLevelEventMTList(
          nextLeveleventMTs);
    }

    LevelOrbitMT levelOrbit =
        new LevelOrbitMT(topoParameter.getID(),
                         level.getNextLevelOrbitHR().getOrbitList(), null);
    levelEvent.nextLevelOrbit = levelOrbit;
    leaves.add(nextLeveleventMTs);
  }

  public void addAddLevel(Application application) {
    // TODO: implement addLevel for ADD type -> NOMODIF only for the time being
    return;
  }

  /**
   * Method to convert a nodeName from HR to an actual dart id (NID) the gmap
   * can recognize when applying a rule
   */
  private void nodeNameToDartID(int AppNumber, String nodeName,
                                JerboaRuleResult appResult,
