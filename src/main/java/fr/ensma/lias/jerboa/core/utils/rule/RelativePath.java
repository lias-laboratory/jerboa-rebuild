	...
	public void computePath() {
		...
		if (this.alphaPath.isEmpty())
			...
			if (detector.isNodeHook(rNode) || detector.isNodeCreated(rNode)) {
				...
				this.alphaPath = ToolKit.collectLabelsFromSourceToClosestTarget(rule, lNode,
						rule.getHooks(), null);
			}
		else {
			...
			path = ToolKit.collectLabelsFromSourceToClosestTarget(rule, lNode, rule.getHooks(),
					hook);
			...
			// left
			path = ToolKit.collectLabelsFromSourceToClosestTarget(rule, hook, Arrays.asList(lNode),
					null);
			alphaPath.addAll(path);
		}
	}
