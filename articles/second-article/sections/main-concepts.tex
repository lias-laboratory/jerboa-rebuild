...
In order to simplify the reading, $\alpha 2$ labels will not be written
in every subsequent figure.

In the same way, faces are split into edges connected with red $\alpha 1$ links (Fig~\ref{fig:demo-fig-show-1-links}).
Lastly, edges themselves are split into vertices with black $\alpha 0$ links (Fig~\ref{fig:demo-fig-show-all-links}) to produce the $2$-G-map describing the objects shown in Fig.~\ref{fig:demo-fig-ABCDE}.
...
For example, the 2-cell adjacent to some dart $a$ (or the object face matching $a$) (Fig.~\ref{fig:demo-fig-2-cell-a}) is the subgraph which contains $a$ and all darts reachable from $a$, using links labeled by $\alpha 0$ or $\alpha 1$ (\textit{i.e.} darts $a$, $b$, $c$, $d$, $e$, $f$, $g$ and $h$) and the links themselves.
This subgraph is denoted by $G\orbit{\alpha 0, \alpha 1}(a)$ and models the face $BCED$ (Fig.~\ref{fig:demo-fig-ABCDE}).
$\orbit{\alpha 0, \alpha 1}$ is the \emph{type} of the orbit.
The $1$-cell adjacent to $a$ (or the object edge matching to  $a$) (Fig.~\ref{fig:demo-fig-1-cell-a}) is the subgraph $G \orbit{\alpha 0, \alpha 2}(a)$ which contains $a$ and all reachable darts using links labeled by $\alpha 0$ or $\alpha 2$ (\textit{i.e.} darts $a, b, l$ and $m$) and the corresponding links.
It represents the topological edge $BC$.
Thanks to $\alpha 2$-loops on the object border, $G\orbit{\alpha 0, \alpha 2}(c)$ (Fig.~\ref{fig:demo-fig-1-cell-c}) also represents the edge $CE$ matching dart $c$.
The $0$-cell adjacent to $a$ (or the object vertex matching dart $a$) (Fig.~\ref{fig:demo-fig-0-cell-a}) is the subgraph $G\orbit{\alpha 1, \alpha 2}(a)$ and represents vertex $B$.
Note that orbits are more general than cells.
For example, the face edge $G\orbit{\alpha 0}(a)$ (Fig.~\ref{fig:demo-fig-1-cell-a0-a}) is an $\orbit{\alpha 0}$ orbit incident to $a$.
...
Informally, in the triangulation rule shown in Fig.~\ref{fig:rule-face-triangulation}, the left side is made of only one node $n_{0}$ labeled with the face $\orbit{\alpha 0, \alpha 1}$ type: this way, it can match any face.
For the match $m : n0 \mapsto a_{0}$ from $L$ to $G$ (Fig.~\ref{fig:demo-fig-triangulation-left-match}), the whole face $G\orbit{\alpha 0, \alpha 1}(a_{0})$ is matched, highlighted in orange in the figure. $\alpha 0$ (resp. $\alpha 1$) links are represented by black (resp. red) segments.
...
Hence, in $H$ (Fig.~\ref{fig:demo-fig-triangulation-right-match}), the $\alpha 0$-links of the matched orange face have been preserved while $\alpha 1$-links have been deleted.
...
For example, in the right-hand side of the triangulation rule (Fig.~\ref{fig:rule-face-triangulation}), the $\orbit{\alpha 0, \alpha 2}$-orbit (an edge) incident to node $n_{0}$ contains the single node $n_0$, and the $\orbit{\alpha 0, \alpha 2}$-orbit incident to $n_1$ contain the two nodes $n_1$ and $n_2$ and the explicit $\alpha_0$-link which connects them.
...
For example, the $\orbit{\alpha 0, \alpha 2}$-orbit incident to $a_0$ in the graph $H$ (Fig.~\ref{fig:demo-fig-triangulation-right-match}) is partially matched by the triangulation rule (only darts $a_0$ and $b_0$ are matched and not their $\alpha_2$ neighbors), because the $\orbit{\alpha 0, \alpha 2}$-orbit incident to $n_0$ in the rule is not complete.
