...
\david{Je te laisse adapter mes phrases, parce que j'utilise le terme instance courante et je ne suis pas sûr que tu t'en serves beaucoup : L'objectif de l'appariement est d'arriver à identifier explicitement les orbites utilisées comme paramètre de la spécification paramétrique initiale dans le modèle réévalué. Cela se fait en utilisant directement les brins de l'instance courante du modèle en cours de réévaluation et non en utilisant des n\oe uds de règles Jerboa qui peuvent filtrer plusieurs instances de brins.}
Les DAGs de réévaluation n'enregistrent donc pas des n\oe uds de règles Jerboa mais directement des brins.
...
\end{figure}
\davidSup{Reprenons l'application} le nom persistant PN\textsubscript{35} du cas d'étude \david{et son DAG d'évaluation (figure...)}, comme point de départ pour
illustrer la création d'un DAG de réévaluation et les différentes étapes
d'appariement des paramètres topologiques. \david{Pour rappel, la spécification paramétrique éditée était présentée listing... et le résultat d'application était illustré sur la figure...} %
...
Nous pouvons alors reproduire \david{dans le DAG de réévaluation} le premier niveau d'application du DAG
...
L'édition de la spécification paramétrique ajoute l'application \texttt{ADD
	10-\allowbreak{}insertion\-Sommet(3)}
...
Dans notre exemple, \davidSup{l'application désigne l'arête incidente au brin $3$, donc nous poursuivons avec la détection des évènements.} l'insertion du sommet sur l'arête incidente au brin \(3\) scinde cette arête en deux. %
...
contient un n\oe ud d'évènement \nomodifnodeevent{}, car l'orbite \(\orbit{}(3)\) (le brin à
l'extrémité de l'arête) est inchangée, et un n\oe ud d'évènement
\splitnodeevent{}, car l'orbite \(\orbit{0}(3)\) (l'arête)\david{on appelle ça arête et pas demi-arête, c'est ce qu'on avait dit pour simplifier la lecture ? }\maxime{on peut dire arête de face, apparemment je l'ai utilisé dans le chapitre de formalisation.} est scindée en deux (figure~
...
l'application \david{suivante dans la spécification paramétrique éditée,} \texttt{INIT 2-\allowbreak{}extrusion\-Face}, pour chaque branche
...
scindées, mais puisque cet évènement était prévu dans l'évaluation initiale \david{et que seule la face ayant pour origine \(\orbit{0}(n_{4})\) était suivie},
alors aucun dédoublement de branche n'est nécessaire, \david{et seules les faces ayant pour origine \(\orbit{0}(37)\) et \(\orbit{0}(52)\) seront suivies dans le DAG de réévaluation}. %
...
L'application ajoutée \texttt{ADD~10-\allowbreak{}insertion\-Sommet} filtre partiellement ces deux
orbites donc nous n'enregistrons pas de \noeffectnodeevent{} dans le DAG \david{mais des \nomodifnodeevent{}}. %
...
Ce suivi des évènements annulés \david{permet de garder trace de toutes les évolutions entre les évènements composant l'histoire d'un nom au jeu initial et ceux survenus au rejeu, et de permettre ainsi la mise en oeuvre de toutes les stratégies d'appariement possibles. Par exemple, }%
...
lorsqu'une orbite référencée dans la spécification paramétrique n'est pas
