...
Ceux-ci présentent en effet l'avantage d'éviter de concevoir un nouvel objet du
début à la fin. %
...
Afin d'être considéré comme efficaces, un mécanisme de réévaluation doit
respecter un certain nombre de contraintes. %
Le mécanisme doit permettre l'identification de manière unique et non ambiguë
des entités topologiques. %
L'objet issu de réévaluation doit respecter les intentions de modélisation de
l'utilisateur, c'est-à-dire que le programme ne doit pas prendre d'initiative à
la place de l'utilisateur. %
L'objet doit être topologiquement et géométriquement cohérent afin de pouvoir
être fabriqué. %
...
évènements sont déterminés indépendamment de l'objet sur lequel une règle est
appliquée. %
Ce formalisme définit également des cas limites où il devient nécessaire de
réaliser, localement, une vérification dans l'objet. %
...
Enfin, afin de répondre aux attentes des utilisateurs, nous proposons deux fonctionnalités~:
la possibilité d'ajouter, supprimer ou déplacer des opérations dans la spécification paramétrique~; et d'adapter la réévaluation à l'aide de stratégies configurables. %
...
nous pouvons identifier un certain nombre de perspectives à court terme. %
...
À moyen terme, nous prévoyons d'étendre l'interface graphique de notre
modeleur. %
Ainsi, lors de la conception d'un modèle, les utilisateurs pourront interagir
avec chacune des fonctionnalités présentées dans ce manuscrit. %
...
opérations. %
Il s'agit, avec une telle contribution, d'associer des dates aux opérations
appliquées et ainsi de réévaluer les opérations suivant un ordre
chronologique. %
Cette piste semble être une perspective crédible dans la mesure où les DAG
permettent déjà d'identifier les conditions de création des paramètres
topologiques. %
Ainsi, nous pouvons, à l'aide de nos DAG, garantir la cohérence des annotations
temporelles pour les opérations d'une spécification paramétrique. %
...
Une autre piste envisagée est l'intégration, dans le modeleur, l'annotation à
...
\maxime{Je suis complètement perdu pour celle-ci, on n'a jamais eu le temps d'en parler donc peut-être que je devrais l'enlever ?}
