...
public class MatchingTree {
    ...
    switch (appType) {
      ...
      case INIT:
        ...
        registerLevel(
            ...
            levelEventHR.getNextLevelOrbit().getOrbitList(),
            appType);
        break;
   ...
   */
  private void computeLevelLists(
      List<NodeEvent> eventList, List<NodeOrbit> orbitList, LevelEventMT newLevelEventMT) {
    ...
    for (NodeEvent nodeEventHR : getLastLevel().get(0).getEventList()) {
      ...
      nodeEventHR.getChild().setChildren(Arrays.asList(new Link(LinkType.TRACE, newNodeEvent)));
