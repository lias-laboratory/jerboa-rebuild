...
n\oe uds d'orbites \orbitnode{\orbit{0}} et \orbitnode{\orbit{0,1}} dans chaque branche
du DAG
...
ud d'orbite \orbitnode{\orbit{0,1}} dans les niveaux d'évènements et d'orbites de
chaque branche
...
topologique de l'application \texttt{INIT~8-coloration(PN\textsubscript{35},
	Bleu)} est terminée. %
...
chanfreinage. %
L'application \texttt{1-carre} produit une face carrée et identifie les orbites
\(\orbit{1}(5)\) et \(\orbit{0,1}(5)\). %
L'application ajoutée \texttt{ADD~9-insert} filtre partiellement ces deux
orbites donc nous n'enregistrons pas de \noeffectnodeevent{} dans le DAG. %
À l'exception de \texttt{DELETE~3-insert}, les autres applications se déroulent
...
de la réévaluation que la suppression de \texttt{3-insert} annule ces deux
...
remplacés. %
En effet, si l'application d'une règle ne modifie une orbite, son annulation ne
...
au niveau d'évènement 4, nous pouvons voir que l'application
\texttt{4-chanfrein} crée un sommet qui n'est pas recréé dans le DAG de
réévaluation
...
Non seulement les DAGs de réévaluation permettent de représenter les historiques
d'évolutions des orbites, ils permettent aussi de représenter les évènements
...
utilisateurs. %
Nous présentons ici plusieurs stratégies de réévaluations qui découlent
directement de l'édition d'une spécification paramétrique avec l'ajout et la
suppression d'opérations. %

\maxime{Dans les paragraphes suivants, je pense à mettre des tableau pour afficher différentes représentations de modèles selon les stratégie de réévaluation.}
...
permet de prévenir le risque de mauvais appariement d'orbites et de produire des
objets qui respectent les intentions de conception d'un utilisateur. %
...
l'application \texttt{4-chanfrein}). %
...
conséquence est que cette face aussi n'est pas re-créée.
De plus, l'orbite sommet \(\orbit{1}\) à partir de laquelle devait être créée
l'arête, même si elle pourrait aisément discriminer une face \(\orbit{0,1}\)
d'une autre, ne correspond pas à une origine de face scindée par la règle de
triangulation. %
Ainsi, un tel appariement ne correspondrait, \textit{a priori}, en rien aux
intentions de conceptions de l'utilisateur %
Au contraire, une alternative consisterait à demander à l'utilisateur
s'il souhaite ou non faire l'appariement sur une telle orbite. %
...
Dans certains cas, la suppression d'une application peut annuler un évènement de
scission, voire de fusion. %
Ces suppressions provoquent alors, indirectement, des effets inverses à ceux des
évènements annulés. %
...
l'application \texttt{3-insert} crée un sommet qui est extrudé en une arête par
l'application \texttt{4-extrusionFaceVolume}. %
L'une des faces, le long de cette arête, est ensuite coloriée en rouge par
l'application \texttt{7-coloration}
...
Dans la spécification paramétrique éditée, l'application \texttt{3-insert} est
supprimée. %
