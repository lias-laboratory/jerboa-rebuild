...
In {\color{red}red}, the vertex orbit $R\orbit{1,2,3}(n_{2})$ incident to
$n_{2}$, matches the vertex $I\orbit{1,2,3}(127)$. %
Since $R\orbit{1,2,3}(n_{2})$ only contains $n_{2}$ which has been created, thus
the orbit itself is \emph{created} and the application of the rule creates the
vertex $I\orbit{1,2,3}(127)$. %
In {\color{blue}blue}, the face vertex $\orbit{1,2}$-orbit incident to the
preserved node $n_{0}$ matches the face vertices of the green face such as the
vertex orbit $H\orbit{1,2}(33)$. %
The node $n_{1}$ is added to the orbit through a $1$-arc, $R\orbit{1,2}(n_{0})$,
thus modifying it. %
Consequently, the matched face's vertices of graph $H$ are modified in graph
$I$. %
Finally, the face edge $\orbit{0}$-orbit incident to the preserved node $n_{0}$
matches the {\color{darkgreen}green} face's edges in $H$. %
Since no node is added nor deleted from the orbit nor any arc is relabelled,
thus the orbit is \emph{not modified}. %

Through this analysis, topological changes can be logged within bulletin boards
which are automatically computed without re\-qui\-ring any other intervention. %
