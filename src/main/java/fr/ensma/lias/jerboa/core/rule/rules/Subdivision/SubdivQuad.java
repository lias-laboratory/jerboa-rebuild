...
public class SubdivQuad extends JerboaRebuiltRule {
  ...
  public SubdivQuad(ModelerGenerated modeler) throws JerboaException {
    ...
    // -------- RIGHT GRAPH
    JerboaRuleNode rn0 =
        new JerboaRuleNode("n0", 0, JerboaOrbit.orbit(-1, 1, 2), 3, new SubdivQuadExprRn0pos());
    JerboaRuleNode rn1 =
        new JerboaRuleNode("n1", 1, JerboaOrbit.orbit(-1, -1, 2), 3, new SubdivQuadExprRn1pos());
    JerboaRuleNode rn3 =
        new JerboaRuleNode("n3", 2, JerboaOrbit.orbit(2, 1, -1), 3, new SubdivQuadExprRn3pos());
  ...
  private class SubdivQuadExprRn0pos implements JerboaRuleExpression {
    ...
    public Object compute(
      ...
      for (JerboaDart face :
          gmap.collect(curleftPattern.getNode(0), JerboaOrbit.orbit(1, 2), JerboaOrbit.orbit())) {
        Vec3 a = new Vec3();
        a.barycenter(
            gmap.<fr.ensma.lias.jerboa.embeddings.Vec3>collect(face, JerboaOrbit.orbit(0, 1), 0));
        faceList.add(a);
      }

      Vec3 F = new Vec3();
      F.barycenter(faceList);
      java.util.List<JerboaDart> edgeList =
          gmap.collect(curleftPattern.getNode(0), JerboaOrbit.orbit(1, 2), JerboaOrbit.orbit(2));
      int k = edgeList.size();
      Vec3 R = new Vec3();
      for (JerboaDart edge : edgeList) {
        R =
            R.addn(
                edge.alpha(0)
                    .<fr.ensma.lias.jerboa.embeddings.Vec3>ebd(0)
                    .addn(edge.<fr.ensma.lias.jerboa.embeddings.Vec3>ebd(0))
                    .div(2.0));
      }
      ...
      return R.mul(2)
          .addn(F)
          .addn(curleftPattern.getNode(0).<fr.ensma.lias.jerboa.embeddings.Vec3>ebd(0).mul((k - 3)))
          .div(k);
  ...
  private class SubdivQuadExprRn1pos implements JerboaRuleExpression {
    ...
    public Object compute(
      ...
      FP1.barycenter(
          gmap.<fr.ensma.lias.jerboa.embeddings.Vec3>collect(
              curleftPattern.getNode(0).alpha(2), JerboaOrbit.orbit(0, 1), 0));
      return curleftPattern
          .getNode(0)
          .<fr.ensma.lias.jerboa.embeddings.Vec3>ebd(0)
          .addn(curleftPattern.getNode(0).alpha(0).<fr.ensma.lias.jerboa.embeddings.Vec3>ebd(0))
          .mul(0.5)
          .addn(FP0)
          .addn(FP1)
          .div(3.0);
