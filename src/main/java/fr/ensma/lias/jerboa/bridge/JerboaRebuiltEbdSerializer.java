...
public class JerboaRebuiltEbdSerializer implements JBAEmbeddingSerialization {
	...
	public JerboaEmbeddingInfo searchCompatibleEmbedding(String ebdname, JerboaOrbit orbit, String type) {
		...
		else if(("point".equals(ebdname) || "position".equals(ebdname) || "pos".equals(ebdname) || "foldPoint".equals(ebdname)) && orbit.equals(modeler.getPos().getOrbit()))
	...
	public CharSequence serialize(JerboaEmbeddingInfo info, Object value) {
		...
		case "color":
			Color c = (Color) value;
			return "" + (c.getRed()/255.f) + " " + (c.getGreen()/255.f) + " " + (c.getBlue()/255.f) + " " + (c.getAlpha()/255.f);
		default:
