...
public class ParametricSpecification {
	...
	public ParametricSpecification(final String displayName, List<Application> applications, int nextApplicationNumber, boolean initialSpec) {
	...
	public void addApplication(JerboaRuleOperation rule, List<PersistentName> PNs,
			List<Integer> dartIDs, ApplicationType appType) throws JerboaException {
		...
		if (this.listener != null) {
			...
			Application specEntry = new Application(this.nextApplicationNumber, rule, PNs, dartIDs, appType, newpos);
			...
			Application specEntry = new Application(this.nextApplicationNumber, rule, PNs, dartIDs, appType, spec.size());
			nextApplicationNumber++;
			spec.add(specEntry);
	...
	public void updateApplicationType(Application app, ApplicationType type) throws JerboaException {
		if (this.spec.contains(app)) {
			...
			System.err.println("ParametricSpecification.updateApplicationType > Application is not in the specification");
			System.exit(-1);
		}
	}
	...
	public void moveApplication(int oldindex, int newindex) throws JerboaException {
		...
		if (this.initialSpec || oldindex == newindex || oldindex < 0 || newindex < 0 || oldindex >= this.spec.size() || newindex >= this.spec.size())
			return;
