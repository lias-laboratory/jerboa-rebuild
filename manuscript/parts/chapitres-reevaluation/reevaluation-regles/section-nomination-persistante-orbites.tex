...
\begin{enumerate}
	...
	\item une flèche \textbf{noire} est créée entre le n\oe ud d'événement
	      (\texttt{SPLIT}) et le n\oe ud d'orbite (\(\orbit{0,1}\)) pour marquer
	      une \emph{relation de suivi}~;
	\item les types d'orbites de suivi (\(\orbit{0,1}\)) et d'origine
	      (\(\orbit{0}\)) sont ajoutés au DAG~;
	\item enfin, le n\oe ud d'événement \texttt{SPLIT} est relié au n\oe ud
	      d'orbite \(\orbit{0,1}\) par une relation de suivi et au n\oe ud
	      d'orbite \(\orbit{0}\) par une \emph{relation d'origine} (représentée
	      par une flèche \textbf{\color{red}rouge}). %
\end{enumerate}
%
...
\begin{enumerate}
	...
	\item nous créons un niveau d'événement, annoté par le numéro d'application et
	      ...
	      \texttt{CREATION} (un pour chaque n\oe ud d'orbite)~;
	\item les n\oe uds d'origine sont ajoutés au DAG~;
	...
	\item enfin, chacun des n\oe uds d'orbite d'origine est relié au n\oe ud
	      d'événement qui convient, c'est-à-dire, l'orbite origine \(\orbit{0}\)
	      (respectivement \(\orbit{}\)) est connectée à l'événement
	      \texttt{CREATION} qui est connecté à l'orbite de suivi \(\orbit{0,1}\)
	      (respectivement \(\orbit{0}\)).
\end{enumerate}
%
Ce niveau du DAG
...
Lorsque nous analysons la règle de contraction, nous pouvons calculer que
l'origine du sommet est la face incidente au n\oe ud \(n_{0}\). %
...
enregistré par l'application jusqu'au n\oe ud d'accroche. %
