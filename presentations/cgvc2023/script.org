...
by modeling operations on objects.

A topological change is an event which, for example, creates, splits, merges, topological cells (faces, edges, vertices) within an object.
...
Typically, these changes are dynamically detected by going through the whole mesh.
Such a detection is done after the actual application of an operation.

Additionally, the more the object is complex the more the event detection is expensive.

*** animation 4
Nowadays, topological events are tracked in every system, for example when
maintaining the consistency of a model during its conception/construction.
...
sharing an edge (the edge BC).
*** animation 2
Firstly, the object is decomposed into faces connected together with a 2-link
(in blue). Where edges are not shared, or borders, there are loops.
*** animation 3
...
** Orbits
Given a set "o" of link types (also known as orbit type) an orbit is the
sub-graph of a g-map incident to a dart.
...
Due to a rewriting of its labels, there is no 0-link between n1's darts and
1-links are replaced by 2-links. The green label is called an implicit link.

There is a 1-link between n0 and n1. It connects n0's darts to n1's darts with
1-links. This link is called an explicit link.
*** animation 6
n2 is also a created node, it creates darts and links in blue.
*** animation 7
While we can define orbits in a gmap, we can do so in rules. Here are the face
orbits matched by the rule before and after its application.
