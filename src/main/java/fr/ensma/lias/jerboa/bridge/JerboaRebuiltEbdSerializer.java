...
public class JerboaRebuiltEbdSerializer implements JBAEmbeddingSerialization {
	...
	@Override
	public JerboaEmbeddingInfo searchCompatibleEmbedding(String ebdname, JerboaOrbit orbit,
			String type) {
		System.out.println("ebdname" + ebdname);
		if (ebdname.equals("color") && orbit.equals(modeler.getColor().getOrbit()))
			return modeler.getColor();
		else if (("point".equals(ebdname) || "position".equals(ebdname) || "pos".equals(ebdname)
				|| "foldPoint".equals(ebdname)) && orbit.equals(modeler.getPos().getOrbit()))
	...
	public CharSequence serialize(JerboaEmbeddingInfo info, Object value) {
		switch (info.getName()) {
			...
			case "color":
				Color c = (Color) value;
				return "" + (c.getRed() / 255.f) + " " + (c.getGreen() / 255.f) + " "
