package fr.ensma.lias.jerboa.Suppression;
...
for(JerboaDart dart : gmap.collect(leftPattern.get(0).getNode(0),JerboaOrbit.orbit(0,1,3),JerboaOrbit.orbit())){
   Integer currentTracker = dart.<fr.ensma.lias.jerboa.embeddings.OrbitLabel>ebd(2).getLabel();
   if(!(visites.contains(currentTracker))) {
      visites.add(currentTracker);
   }
}
