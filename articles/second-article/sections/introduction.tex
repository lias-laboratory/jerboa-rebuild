...
\maxime{Number of applications already make use of Jerboa and/or G-maps in
  fields like architecture [], geology [] or physics-based modelling [].} %
%
...
\maxime{Such a system can be thought as a dual structure with, in one hand, the
  ...
  the successive operation (and their paramters) used for the object's
  modelling.} %
%
...
\maxime{The persistent naming problem has been studied for decades in the CAO's
  field, yet none has been written jointly with the use of graph transformation
  rules.} %
%
...
paramétriques. Pour cela nous proposons une méthode de nommage persistant qui
tire parti de la formalisation des opérations via les règles et de leur capacité
à décrire précisément l’histoire des entités topologiques pour caractériser ces
entités de manière unique et homogène en toutes dimensions, et cela, quelle que
soit la dimension du modèle. D’une manière ou d’une autre, la plupart des
méthodes existantes nécessitent de suivre l’évolution de nombreuses entités
topologiques et abordent le problème de ma persistance des noms uniquement dans
le cadre de la modification des paramètres de la spec [Kripac, Chen, Wu, etc.].
...
édition de la spec). Dans la section 2, nous présenterons les mains concepts
nécessaires à la mise en œuvre de mécanismes de nommage persistants dans le
cadre d’un système utilisant des règles de récriture de graphe. En particulier,
nous présentons les g-maps, les règles et la manière dont il est possible de
détecter automatiquement, lors de l’application d’une règle, les évènements
(création, suppression, scission, fusion, modification) survenus sur les entités
topologiques. Dans la section 3, nous présentons la structure de la
spécification paramétrique et des noms persistants. La section 4 et 5, aborde
ensuite la notion d’Arbres de jeu et de rejeu qui permettent d’une part de
tracer l’évolution des entités topologiques désignées dans la spécification
paramétrique et des entités dont elles sont issues et d’autre part de trouver
les entités qui leur correspondent au rejeu.
